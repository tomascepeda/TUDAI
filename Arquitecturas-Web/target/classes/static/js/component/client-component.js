...
class ClientComponent extends TemplateComponent{
    ...
    loadJs() {
        function getHtml(c, cp=[undefined]) {
            return `
            <li clas="list-item" id="client-${c.id}">
                ${getClients(c, cp)}
                ${getModals(c)}
            </li>`;
        }
            ...
            return `
                <div class="client">
                    ...
                    <div class="product-buttons">
                        <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#modal-edit-id-${c.id}">Editar</button>
                        <button type="button" class="btn btn-danger" data-method="DELETE" data-id="${c.id}">Borrar</button>
                    </div>
                </div>`;
        }
        ...
        function getModals(c) {
            return `
            <div class="collapse" id="modal-edit-id-${c.id}">
                ${getForms(c)} 
            </div>`;
        }
        function getForms(data) {
            ...
            <form action="#" data-method="PUT">
                <div class="form-group">
                    ...
                    <input type="text" name="id" disabled value="${(data)?data.id:''}"/>
        ...
        async function render(page="", size="") {
            ...
            await clients.elements.forEach(async (client, i) => {
                const products = await Util.fetchJson(productsBoughtEndPoint+ client.id); 
                    ...
                    await el.addEventListener("submit-request", async (e)=> {
                        ...
                        switch(e.detail.method) {
                            ...
                            case 'PUT':
                                ...
                                break;
                            case 'DELETE':
                                await Util.remove(`clients/${e.detail.formData.id}`);
                                break;          
                            default:
                                break;
                        }
